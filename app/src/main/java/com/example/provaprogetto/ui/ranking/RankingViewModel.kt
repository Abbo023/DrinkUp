        ...
        viewModelScope.launch {
            try {
                if (recipeCache != repository.getCustomRecipes()) {
                    val results = repository.getCustomRecipes().sortedByDescending { it.likes }
                    recipeCache = results
                    Log.d("RankingViewModel", "$recipeCache")

                    _recipe.postValue(recipeCache!!)
                    Log.d("RankingViewModel", "Ricette caricate con successo")
                } else {
                    _recipe.postValue(recipeCache!!)
                    Log.d("RankingViewModel", "Drink e ricette caricate con la cache")
                }
            } catch (e: Exception) {
                e.printStackTrace()
            }
    ...
    fun searchDrinksByIngredientsAndName(
        ...
        viewModelScope.launch {
            val filteredRecipes = recipeCache?.filter { recipe ->
                val nameMatches = if (drinkName != null) {
                    recipe.name.contains(drinkName, ignoreCase = true)
                }
                else {
                    true
                }
            ...
            if (filteredRecipes != recipeCache) {
